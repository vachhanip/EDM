...
<body>
    <form method="post">
        ...
        <div style="margin-left:15%; padding:1px 16px;">
            ...
            <input type="text" class="atex" name="category" placeholder="Category Name" required>
            <input type="submit" class="abtn" name="submit" value="Add Category">
            <?php
                require '../config/db.php';

                if (isset($_POST["submit"])) {
                    $category = $_POST['category'];
                    $addc = mysqli_query($conn,"INSERT INTO categorie (name) VALUES ('$category')");
                    if ($addc) {
                        header("location:view_category.php");
                    } else {
                        echo "Error: " . $addc . "<br>" . $conn->error;
                    }
                }
            ?>
        </div>
